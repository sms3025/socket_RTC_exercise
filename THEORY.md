...
### 5.5 Mesh vs SFU — 인원이 늘어날 때

- **Mesh(그물망)**: 모두가 모두와 1:1 연결. N명이면 각자 N-1개 연결 → 인원 제곱으로 부담 증가. **소규모(4~6명)에 적합.** (이 프로젝트 방식)
- **SFU(Selective Forwarding Unit)**: 각자 서버에 한 번만 올리고, 서버가 나머지에게 배분. mediasoup·LiveKit 등. **인원이 많을 때** 필요. (STUDY.md의 확장 항목)
