...
### 5-B.4 미디어 on/off — "내가 끄기" vs "서버가 강제로 끄기" (5단계)

화상 서비스에는 두 종류의 "끄기"가 있다. 이 둘을 구분하는 게 5단계의 핵심이다.

| 구분 | 누가 | 어떻게 | 이 프로젝트 위치 |
|------|------|--------|------------------|
| **내가 끄기** | 참가자 본인 | 클라이언트 SDK 로 내 트랙을 mute | `livekit.html` `toggleCamera/toggleMic` |
| **강제로 끄기** | 서버(진행자) | 서버가 LiveKit 에 관리 명령 | `LiveKitAdminController` |

**(1) 내가 끄기 — 클라이언트 제어**

```js
room.localParticipant.setCameraEnabled(false);      // 내 카메라 OFF
room.localParticipant.setMicrophoneEnabled(false);  // 내 마이크 OFF
```
- SDK 가 해당 트랙을 mute 하고 "이제 이 트랙 안 보낸다"를 LiveKit 서버에 알린다.
- 현재 상태는 `isCameraEnabled` / `isMicrophoneEnabled` 로 읽어 버튼 글자를 맞춘다.

**(2) 서버가 강제로 끄기 — `RoomServiceClient`**

토큰 발급(4단계)이 "브라우저에게 입장권을 내주는" 일이었다면, 강제 제어는 **우리 서버가
LiveKit 서버에게 직접 관리 명령(REST)을 보내는** 일이다. 방향이 반대다.

```
[4단계] 브라우저 ──토큰 요청──▶ 우리 서버 ──토큰 발급──▶ 브라우저   (브라우저를 향함)
[5단계] 진행자UI ──강제OFF요청─▶ 우리 서버 ──mute 명령──▶ LiveKit 서버 (LiveKit 을 향함)
```

- **핵심 API**: `RoomServiceClient.mutePublishedTrack(room, identity, trackSid, muted)`
  - 절차: ① `getParticipant(room, identity)` 로 대상의 트랙 목록을 받고 → ② `source`(CAMERA/
    MICROPHONE) 가 일치하는 트랙의 `trackSid` 를 찾아 → ③ 그 sid 로 mute 명령을 보낸다.
  - `trackSid` 가 필요한 이유: 한 사람이 카메라·마이크·화면공유 등 **여러 트랙**을 동시에 발행할
    수 있어서, "어느 트랙"인지 딱 집어줘야 하기 때문.
- **왜 강제가 통하나?** SFU 에서는 모든 미디어가 LiveKit 서버를 통과하므로, 서버가 "이 트랙 그만
  보내"라고 하면 실제로 끊긴다. 대상 브라우저는 아무 코드도 안 짰지만 SDK 가 `TrackMuted` 이벤트를
  받아 자동으로 반영한다. **클라이언트가 조작해도 서버가 최종 결정권을 쥔다** — 이게 4단계에서
  배운 "SFU 는 서버가 규칙을 강제할 수 있다"의 구체적 사례다.

> ⚠️ **강제 "켜기(unmute)" 는 기본적으로 제한된다.** 동의 없이 남의 마이크를 켜면 도청이 되므로,
> LiveKit 은 서버발(發) unmute 를 프라이버시 이유로 막아둔 경우가 많다(개발 모드에선 대체로 허용).
> 실무에서는 "서버가 강제로 켜기"보다 **"서버가 '켜 달라'는 신호만 보내고 최종 켜기는 본인이
> 누르게"** 설계하는 것이 안전하다.

**마피아 게임 응용**
- 죽은 사람 → 진행자가 그 사람 **마이크 강제 OFF** (말 못 하게)
- 밤이 되면 → 전원 **카메라 강제 OFF** (서로 얼굴 못 보게)
- 이런 통제가 Mesh(2·3단계)에선 불가능했다. 미디어가 서버를 안 거쳐서 끊을 지점이 없기 때문.

**핵심 질문**
- "내가 끄기"와 "서버가 강제로 끄기"는 각각 어디서(클라/서버) 어떤 API 로 이뤄지는가?
- `mutePublishedTrack` 에 왜 `trackSid` 가 필요한가? identity 만으로는 왜 부족한가?
- 서버가 강제로 껐을 때, 대상 브라우저의 버튼은 어떻게 저절로 "켜기"로 바뀌는가? (힌트: `TrackMuted`)
- 강제 "켜기"가 강제 "끄기"보다 위험한 이유는? 실무에선 어떻게 우회 설계하나?
